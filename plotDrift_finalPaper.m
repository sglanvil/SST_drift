...
printName='drift_cesm1_e3sm_kirtmanFix_HadISST_finalPaper';
...
panelLetter={'a','e','i','m','b','f','j','n','c','g','k','o','d','h','l','p','q','r','s','t'};
...
for iloop=1:2
    for imodel=1:2
        ...
        % March 3, 2023
        % calculate R(year5,LE)
        replat=repmat(lat(abs(lat)<90),1,128)';
        [R,P]=corrcoef(diff(:,abs(lat)<90).*cosd(replat),...
            squeeze(diff_year5_save(:,abs(lat)<90,icounter)).*cosd(replat),...
            'rows','pairwise');
        disp(R(1,2))
